...
public class CommitLog {
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            if (msg.getDelayTimeLevel() > 0) {
                ...
                // Backup real topic, queueId 原topic与queueId存入property
                MessageAccessor.putProperty(msg, MessageConst.PROPERTY_REAL_TOPIC, msg.getTopic());
                MessageAccessor.putProperty(msg, MessageConst.PROPERTY_REAL_QUEUE_ID, String.valueOf(msg.getQueueId()));
                msg.setPropertiesString(MessageDecoder.messageProperties2String(msg.getProperties()));

                // 用延时topic与queueId替换掉原先的
                msg.setTopic(topic);
    ...
    /**
     * commitLog刷盘  刷盘的异步与同步的区别在于：同步指的是消息存储后就同步调用force将此数据刷入磁盘（此逻辑在另一线程中）；异步指的是消息存储后
     * 只需要唤醒刷盘线程就可直接返回（刷盘线程中异步执行任务）。此处的刷盘同步与异步是消息存储与flush()之间的关系
     * @param result
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                ...
                try {
                    // 同步等待，时间为this.defaultMessageStore.getMessageStoreConfig().getSyncFlushTimeout()
    ...
    /**
     *
     * @param offset:消息全局物理offset
     * @param size:消息长度
     * @return
     */
    public SelectMappedBufferResult getMessage(final long offset, final int size) {
        int mappedFileSize = this.defaultMessageStore.getMessageStoreConfig().getMappedFileSizeCommitLog();
        // 根据消息offset获取所在MappedFile
    ...
    public long rollNextFile(final long offset) {
        // 一个文件的容量
        int mappedFileSize = this.defaultMessageStore.getMessageStoreConfig().getMappedFileSizeCommitLog();
        // fileFromOffset应该一直是mappedFileSize的整数倍吧，这里%是为了干啥？--为了兼容offset不是fileFromOffset的场景，offset可能是文件中的某一条消息的偏移量
    ...
    class GroupCommitService extends FlushCommitLogService {
        // 将提交与执行分开，避免锁冲突，提高并发（在提交刷盘任务的同时可以执行刷盘）
        // 任务提交
            ...
            synchronized (this.requestsRead) {
                if (!this.requestsRead.isEmpty()) {
                    for (GroupCommitRequest req : this.requestsRead) {
                        ...
                        // 当前req的flush完成后，future结果更新，观察者模型
        ...
        public void run() {
            ...
            // 当前对象加锁  requestsWrite的add不能与swapRequests同时执行，在add任务后再交换，从而去执行，但是如果执行过程中发生了交换呢？--交换与执行是同一个线程执行的，不会冲突
            synchronized (this) {
                // 交换
                this.swapRequests();
            }
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        // File at the end of the minimum fixed length empty 高四字节存储当前文件剩余空间、低四字节存储魔数
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // PHY OFFSET 此时byteBuffer中即将写入数据的offset(全局)
            ...
            String msgId;
            // 构造msgId,msgId中包括了消息偏移量(commitlog文件偏移+文件中的偏移)，从而应用得到msgId时可以快速索引到具体消息数据
            if ((sysflag & MessageSysFlag.STOREHOSTADDRESS_V6_FLAG) == 0) {
            ...
            // 每个commitlog文件至少有END_FILE_MIN_BLANK_LENGTH字节空闲
            // Determines whether there is sufficient free space
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) {
                // 如果当前文件空闲空间不够了，则返回异常，broker会重新创建新的commitlog文件来存储该消息
