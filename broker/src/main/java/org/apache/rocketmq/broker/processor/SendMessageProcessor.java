...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    /**
     * 处理请求，从NettyServerHandler.channelRead0()触发
     * @param ctx
     * @param request
     * @return
    ...
    private CompletableFuture<RemotingCommand> asyncConsumerSendMsgBack(ChannelHandlerContext ctx,
        ...
        // 3. 根据commitLogOffset从commitLog中获取消息
        ...
        if (msgExt.getReconsumeTimes() >= maxReconsumeTimes 
            || delayLevel < 0) {
            // 4. 判断如果该消息重试次数已经超过配置，则改变topic为DLQ（该主题的权限只写，说明不再被消费了）
    ...
    private CompletableFuture<RemotingCommand> asyncSendMessage(ChannelHandlerContext ctx, RemotingCommand request,
        ...
        // queueId(这是producer发送时选择的queue--其实并没有真正的java中的queue数据结构，myConfusion:那这里的queueId具体是如何起作用的呢？)
        ...
        if (transFlag != null && Boolean.parseBoolean(transFlag)) {
            ...
            // 备份msg原主题与原消息队列，然后将主题变更为RMQ_SYS_TRANS_HALF_TOPIC,将消息存储到commitLog和RMQ_SYS_TRANS_HALF_TOPIC
            // 之下的consumeQueue。此消息由于不在原topic之下，因此不会被消费，类似定时消息的处理
    ...
    private boolean handleRetryAndDLQ(SendMessageRequestHeader requestHeader, RemotingCommand response,
        ...
        if (null != newTopic && newTopic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX)) {
            ...
            if (reconsumeTimes >= maxReconsumeTimes) {
                // 如果消费次数大于maxReconsumeTimes，则消息进入DLQ延迟队列--myConfusion:具体的队列在哪？只是改了消息的topic吗？
